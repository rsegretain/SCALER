...
def custom_filelist_filter(files_list):
	files_list = list(filter(lambda file : "mongo" not in file.split('/')[-1] , files_list))
	files_list = list(filter(lambda file : "memcached" not in file.split('/')[-1] , files_list))
	files_list = list(filter(lambda file : "consul" not in file.split('/')[-1] , files_list))
	files_list = list(filter(lambda file : "jaeger" not in file.split('/')[-1] , files_list))
	return files_list
...
utils.plot_avg_cpu_per_replicas(global_result_folder_path, plots_folder_path, getServiceNameFromPodName, subtitle, CPU_LIMIT, caption=caption, filter=custom_filelist_filter)
...
try:
	...
	dir_list = list(filter(lambda file : "consul" not in file.split('/')[-1] , dir_list))
...
utils.plot_latency(global_result_folder_path, plots_folder_path, getServiceNameFromPodName, subtitle, MAX_LATENCY, caption=caption, filter=custom_filelist_filter)
...
try:
	...
	replicas_datas.drop(columns=["consul", "jaeger", "mongodb-recommendation", "memcached-profile", "mongodb-profile", "memcached-reserve", "mongodb-user", "mongodb-rate", "memcached-rate", "mongodb-reservation", "mongodb-geo"], inplace=True)
	# print(replicas_datas.columns)
	...
	replicas_ready_datas.drop(columns=["consul", "jaeger", "mongodb-recommendation", "memcached-profile", "mongodb-profile", "memcached-reserve", "mongodb-user", "mongodb-rate", "memcached-rate", "mongodb-reservation", "mongodb-geo"], inplace=True)
